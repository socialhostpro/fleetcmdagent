...
export const setNodeCluster = async (nodeId, cluster) => {
  const response = await api.post(`/swarm/nodes/${nodeId}/labels`, {
    labels: { cluster: cluster, nvidia: 'true' }
  });
  return response.data;
};

export const removeNodeFromCluster = async (nodeId) => {
  await api.delete(`/swarm/nodes/${nodeId}/labels/cluster`);
